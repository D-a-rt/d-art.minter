...
type get_owners = transfer_description -> (address option) list

type hook_entry_point = transfer_description_param contract
...
@param get_owner selector of `from_` or `to_` addresses from each individual `transfer_description`
 *)
let get_owners_from_batch (batch, get_owners : (transfer_description list) * get_owners) : address set =
  List.fold
    (fun (acc, tx : (address set) * transfer_description) ->
...
let validate_owner_hook (p, get_owners, to_hook, is_required :
    transfer_description_param * get_owners * to_hook * bool)
...
let validate_owner(p, policy, get_owners, to_hook :
    transfer_description_param * owner_hook_policy * get_owners * to_hook)
    : hook_entry_point list =
  match policy with
...
cannot be met.
 *)
let validate_receivers (p, receiver_policy : transfer_description_param * owner_hook_policy)
    : hook_entry_point list =
  let get_receivers : get_owners = fun (tx : transfer_description) ->
    List.map (fun (t : transfer_destination_descriptor) -> t.to_ )tx.destinations in
...
cannot be met.
 *)
let validate_senders (p, sender_policy : transfer_description_param * owner_hook_policy)
    : hook_entry_point list =
  let get_sender : get_owners = fun (tx : transfer_description) -> [tx.from_] in
 ...
 *)
let get_owner_transfer_hooks (p, descriptor : transfer_description_param * permissions_descriptor)
    : hook_entry_point list =
...
let transfers_to_descriptors (transfers : transfer list) : transfer_description list =
  List.map
    (fun (transfer : transfer) ->
      let transfers = List.map
        (fun (destination : transfer_destination) ->
          {
            to_ = Some destination.to_;
            token_id = destination.token_id;
            amount = destination.amount;
          }
        ) transfer.destinations in
        {
          from_ = Some transfer.from_;
          destinations = transfers;
        }
    ) transfers

let transfers_to_transfer_descriptor_param
    (transfers, operator : (transfer list) * address) : transfer_description_param =
  {
    batch = transfers_to_descriptors transfers;
    operator = operator;
  }
...
let get_owner_hook_ops_for (tx_descriptor, pd
    : transfer_description_param * permissions_descriptor) : operation list =
