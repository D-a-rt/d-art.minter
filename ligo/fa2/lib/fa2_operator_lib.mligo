...
let validate_update_operators_by_owner (update, updater : update_operator * address)
    : unit =
  match update with
    | Add_operator op -> if op.owner = updater then unit else (failwith fa2_not_owner : unit)
    | Remove_operator op -> if op.operator = updater || op.owner = updater then unit else (failwith fa2_not_owner : unit)
